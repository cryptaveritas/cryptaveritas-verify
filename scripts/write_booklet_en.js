...
:root{
  ...
  --text:#fafafa;--muted:#a1a1aa;--hint:#71717a;--accent:#06b6d4;--accent2:#8b5cf6;
...
.card-tag{font-family:var(--mono);font-size:10px;letter-spacing:2px;text-transform:uppercase;margin-bottom:12px;color:var(--hint)}
...
.code-block{background:var(--surface);border:1px solid var(--border);padding:24px;margin-top:24px;font-family:var(--mono);font-size:12px;color:var(--muted);line-height:1.8;overflow-x:auto}
.code-block .comment{color:var(--hint)}.code-block .keyword{color:var(--accent2)}.code-block .value{color:var(--accent)}
...
.tiers{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--border);margin-top:40px}
...
.tier h3{font-size:20px;margin-bottom:12px}.tier-val{font-family:var(--mono);font-size:13px;color:var(--accent);margin-bottom:14px}.tier p{font-size:13px}
...
.repos{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:32px}
...
.footer{padding:48px 0;display:flex;justify-content:space-between;align-items:center}
...
.nav{position:fixed;top:0;left:0;right:0;background:rgba(9,9,11,0.9);backdrop-filter:blur(12px);border-bottom:1px solid var(--border);z-index:100;padding:0 32px}
...
.nav-links a{font-family:var(--mono);font-size:11px;color:var(--hint);letter-spacing:1px;text-transform:uppercase;text-decoration:none}
...
@media(max-width:600px){
  .cards-2,.cards-3,.steps,.tiers{grid-template-columns:1fr}
...
<div class="wrap">
  ...
  <p class="hero-desc">Cryptographic verification protocol for trading signals. Every signal is committed before reveal — mathematically, without trusting the source.</p>
  <div class="hero-pills">
    <div class="pill">Token <span>$CRYSIG (Solana)</span></div>
    <div class="pill">License <span>GPL v3.0</span></div>
    <div class="pill">Status <span>Phase 1 complete</span></div>
    <div class="pill">Tests <span>14/14 passing</span></div>
  </div>
</div>
</div>
...
<div class="wrap">
  ...
  <div class="cards cards-2">
    <div class="card">
      <div class="card-tag red">Problem</div>
      <h3>Fabricated track records</h3>
      <p>Signal sources delete losing forecasts, change entry points retroactively, and publish different signals to different groups. Users have no tool for independent verification.</p>
    </div>
    <div class="card">
      <div class="card-tag green">Solution</div>
      <h3>Commit-Reveal protocol</h3>
      <p>Before publishing a signal, its SHA-256 hash with salt is recorded. After reveal, anyone can recompute the hash and confirm: the signal was not altered. Mathematical proof, not reputational.</p>
    </div>
  </div>
  <div class="slogan">
    <div class="slogan-text">"Every signal. Cryptographically proven."</div>
    <div class="slogan-sub">B2B slogan</div>
  </div>
</div>
</div>
...
<div class="section">
<div class="wrap">
  <div class="sec-label">02 — How the protocol works</div>
  <h2>Three steps.<br>Zero trust.</h2>
  <div class="steps">
    <div class="step">
      <div class="step-num">01</div>
      <h3>Commit</h3>
      <p>Signal source publishes SHA-256 hash with domain prefix and random salt. Signal content is unknown to anyone.</p>
      <code>cryptasignals:v1|signal|{data}|{salt}</code>
    </div>
    <div class="step">
      <div class="step-num">02</div>
      <h3>Reveal</h3>
      <p>Worker decrypts AES-256-GCM, verifies hash, and publishes the signal with result to the channel.</p>
      <code>VERIFIED / INVALID_HASH / NO_SECRET</code>
    </div>
    <div class="step">
      <div class="step-num">03</div>
      <h3>Verify</h3>
      <p>Anyone can recompute the hash independently in browser or via CLI. No server trust required.</p>
      <code>cryptaveritas.github.io/cryptaveritas-verify</code>
    </div>
  </div>
</div>
</div>
...
<div class="wrap">
  ...
  <h2>Production-ready.<br>Node.js 20 LTS.</h2>
  <div class="tbl-wrap">
    <table>
      ...
      <tr><td>Validation</td><td>Zod .strict() + .refine()</td></tr>
      <tr><td>Bot</td><td>grammY (Telegram) — primary + backup</td></tr>
      <tr><td>Testing</td><td>Jest + ts-jest — 14/14 tests passing</td></tr>
      <tr><td>Token</td><td>Solana SPL — $CRYSIG</td></tr>
    </table>
  </div>
  <div class="code-block">
<span class="comment">// Core architecture</span>
[AI Agent / External API]
  |
  v
[Crypto Core] <span class="keyword">-></span> createCommitment() <span class="keyword">-></span> strictData + salt <span class="keyword">-></span> domainHash
  |
  +<span class="keyword">-</span> Public Telegram channel <span class="comment">(hash published)</span>
  +<span class="keyword">-</span> SQLite WAL <span class="comment">(AES-256-GCM encrypted secret)</span>
  |
  v
[Worker] <span class="keyword">-></span> processPendingCommitments() every 10s
  |
  +<span class="keyword">-</span> Check reveal_deadline
  +<span class="keyword">-</span> decryptSecret <span class="keyword">-></span> verifyCommitment(hash, object, salt)
  +<span class="keyword">-</span> markAsRevealed(hash, status, NORMAL / FORCED)
  +<span class="keyword">-</span> Graceful Shutdown + wal_checkpoint(TRUNCATE)</div>
</div>
</div>
...
<div class="wrap">
  ...
  <h2>10 attack vectors.<br>10 closed vulnerabilities.</h2>
  <div class="sec-grid">
    ...
    <div class="sec-item"><div class="sec-dot"></div><div><div class="sec-title">Key memory leak</div><div class="sec-desc">clearMasterKey() — Buffer.fill(0)</div></div></div>
    <div class="sec-item"><div class="sec-dot"></div><div><div class="sec-title">Overlapping worker runs</div><div class="sec-desc">isProcessing flag in setInterval</div></div></div>
...
<div class="section">
<div class="wrap">
  <div class="sec-label">05 — Test results</div>
  <h2>14 tests. 3 suites.<br>All green.</h2>
  <div class="test-box">
    <div class="test-header">Test Suites: 3 passed &nbsp;|&nbsp; Tests: 14 passed &nbsp;|&nbsp; Node.js 20 LTS</div>
    <div class="test-suite">
      <div class="test-suite-name">crypto.test.ts — 4 tests</div>
      <div class="test-case">toStrictString is deterministic (key order)</div>
      <div class="test-case">domainHash is stable and unique</div>
      <div class="test-case">verifyCommitment is correct (true/false)</div>
      <div class="test-case">createCommitment generates compatible data</div>
    </div>
    <div class="test-suite">
      <div class="test-suite-name">revealWorker.test.ts — 5 tests</div>
    ...
    <div class="test-suite">
      <div class="test-suite-name">database.test.ts — 5 tests</div>
...
<div class="wrap">
  ...
  <div class="roadmap">
    <div class="rm-item">
      <div class="rm-phase"><span class="num">01</span><span class="time">May 2026</span></div>
      <div>
        <div class="badge badge-done">Complete</div>
        <h3>CryptaSignals MVP</h3>
        <p>Commit-Reveal crypto core, AES-256-GCM, SQLite WAL, Telegram bot (primary + backup), HTTP API, public verifier, 14 tests, GitHub Pages.</p>
      </div>
    </div>
    <div class="rm-item">
      <div class="rm-phase"><span class="num">02</span><span class="time">Weeks 3-8</span></div>
      <div>
        <div class="badge badge-next">Next</div>
        <h3>Growth and monetization</h3>
        <p>Token-gated access via $CRYSIG, B2B Pro/Enterprise with commercial license, Drip Reveal, multi-oracle (Birdeye + Pyth), legal entity registration on trigger.</p>
      </div>
    </div>
    <div class="rm-item">
      <div class="rm-phase"><span class="num">03</span><span class="time">Months 3-6</span></div>
      <div>
        <div class="badge badge-plan">Planned</div>
        <h3>Infrastructure migration</h3>
        <p>Onchain pre-commit hashes on Solana, agent reputation ERC-8004 on Base, staking contract, verified signal source marketplace, Soulbound NFTs.</p>
      </div>
    </div>
    <div class="rm-item">
      <div class="rm-phase"><span class="num">04</span><span class="time">Months 6-12+</span></div>
      <div>
        <div class="badge badge-plan">Planned</div>
        <h3>Full CryptaVeritas protocol</h3>
        <p>ZK proofs (Noir, zkVerify), decentralized validator network with slashing, onchain credit scoring for AI agents, DeFAI protocol partnerships.</p>
      </div>
    </div>
  </div>
</div>
...
<div class="wrap">
  <div class="sec-label">07 — Monetization model</div>
  <h2>Token as access.<br>Not as investment.</h2>
  <div class="tiers">
    <div class="tier">
      <div class="tier-name">B2C</div>
      <h3>Private signals</h3>
      <div class="tier-val">Hold 1,000 $CRYSIG</div>
      <p>Offchain balance check via Helius RPC. No fiat payments. No contracts.</p>
    </div>
    <div class="tier">
      <div class="tier-name">B2B Lite</div>
      <h3>Basic API</h3>
      <div class="tier-val">Hold 10,000 $CRYSIG</div>
      <p>Access to verification API. Phase 3 — staking contract on Solana.</p>
    </div>
    <div class="tier">
      <div class="tier-name">B2B Pro / Enterprise</div>
      <h3>Commercial license</h3>
      <div class="tier-val">Legal entity + fiat</div>
      <p>White label, SLA, extended API, ZK verification in Phase 4. Only after registration.</p>
    </div>
  </div>
</div>
</div>
...
<div class="section">
<div class="wrap">
  <div class="sec-label">08 — $CRYSIG tokenomics</div>
  <h2>10,000,000 CRYSIG.<br>Solana.</h2>
  <div class="token-grid">
    ...
    <div class="token-row"><div><div class="token-label">Treasury</div><div class="token-sub">Multisig 2 of 3</div></div><div class="token-val">25%</div></div>
...
<div class="section">
<div class="wrap">
  <div class="sec-label">09 — Competitive landscape</div>
  <h2>No direct competitors.<br>Niche is open.</h2>
  <div class="tbl-wrap">
    <table>
      <tr><th>Project</th><th>Approach</th><th>Difference</th></tr>
      <tr><td>Alpha Impact</td><td>Post-fact verification via onchain trades</td><td>Does not commit signal BEFORE reveal</td></tr>
  ...
  <div class="cards cards-3" style="margin-top:20px">
    ...
    <div class="card"><div class="card-tag red">Rely on signals</div><h3>58%</h3><p style="font-size:13px">From social media — unverified</p></div>
  </div>
</div>
</div>
...
<div class="section">
<div class="wrap">
  <div class="sec-label">10 — Legal structure</div>
  <h2>Trigger model.<br>No unnecessary costs.</h2>
  <div class="triggers">
    <div class="trigger"><div class="trigger-num">00</div><div><h3>Now — pre-launch testing</h3><p>Operating without legal entity. No real payments, no commercial agreements. Second token fully isolated from protocol.</p></div></div>
    <div class="trigger"><div class="trigger-num">01</div><div><h3>First B2B contact with payment intent</h3><p>Immediate registration of legal entity (~200 EUR, 1 business day). Do not wait for actual payment.</p></div></div>
    <div class="trigger"><div class="trigger-num">02</div><div><h3>First incoming payment</h3><p>Open dedicated business account. All payments through legal entity only. Stablecoins on multisig 2 of 3.</p></div></div>
    <div class="trigger"><div class="trigger-num">03</div><div><h3>Cumulative revenue 5,000 EUR</h3><p>Legal opinion on $CRYSIG (MiCA compliance). Decision on second token integration based on opinion.</p></div></div>
  </div>
</div>
</div>
...
<div class="section">
<div class="wrap">
  <div class="sec-label">11 — Repositories</div>
  <h2>Open verifier.<br>Closed core.</h2>
  <div class="repos">
    <div class="repo"><div class="repo-type">Private</div><div class="repo-name">cryptasignals-phase1</div><p>Full core: TypeScript, SQLite, AES-256-GCM, Telegram bot, HTTP API.</p></div>
    <div class="repo"><div class="repo-type">Public — GPL v3.0</div><div class="repo-name">cryptaveritas-verify</div><p>Hash verifier: index.html (browser) + cli-verify.js (Node.js).</p></div>
...
<div class="footer">
  ...
  <div class="footer-info">
    <div>github.com/cryptaveritas</div>
    <div><a href="index.html">cryptaveritas.github.io/cryptaveritas-verify</a></div>
...
fs.writeFileSync('booklet.html', html, 'utf8');
